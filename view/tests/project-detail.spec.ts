...
import { getProjectRowByName, getMainContent } from "./helpers.ts";

test.describe("Project Detail", () => {
  test("shows start page when no project is selected", async ({ dacmPage }) => {
  ...
  test("shows static detail when project is selected", async ({ dacmPage }) => {
    await getProjectRowByName(dacmPage, "Fix pagination bug").click();

    const main = getMainContent(dacmPage);
    await expect(main.locator(".project-detail-name")).toHaveText("Fix pagination bug");
  ...
  test("shows description when present", async ({ dacmPage }) => {
    ...
    const main = getMainContent(dacmPage);
    await expect(main.locator(".project-detail-section", { hasText: "Description" })).toContainText("Users report page 2");
  });

  test("shows iteration count", async ({ dacmPage }) => {
    await getProjectRowByName(dacmPage, "Add authentication flow").click();

    const main = getMainContent(dacmPage);
    await expect(main.locator(".project-detail-section", { hasText: "Iterations" })).toContainText("3");
  });

  test("archive button removes project from sidebar", async ({ dacmPage }) => {
    await getProjectRowByName(dacmPage, "Fix pagination bug").click();
